...
#define HOME_SCREEN_INIT                  1
#define HOME_SCREEN                       2
#define HALL_PROBES_SCREEN                3
#define DATA_INPUT_SCREEN_INIT            4
#define DATA_INPUT_SCREEN                 5
...
#define RECEIVE_ID_00                     0
#define RECEIVE_ID_01                     1
#define RECEIVE_ID_02                     2
#define RECEIVE_ID_03                     3
#define RECEIVE_ID_04                     4
#define RECEIVE_ID_05                     5
#define RECEIVE_ID_06                     6
#define RECEIVE_ID_07                     7
...
int screenState = SPLASH_SCREEN; // start at home screen init state
...
int remote_period = 1000;
int splash_period = 2000;
unsigned long time_now_display = 0;
unsigned long time_now_adc = 0;
unsigned long time_now_stepper = 0;
unsigned long time_now_remote = 0;
unsigned long time_now_splash = 0;
...
void loop()
{
  switch(screenState)
  {
    case SPLASH_SCREEN:
      ...
      delay(200);
      if(millis() >= (time_now_splash + splash_period))
      {
        time_now_splash = millis();
...
int readRotaryEncoder()
{
  ...
  if((int)encoder1.getDirection() != 0 || remote_enc_1 == 1)
  {
    remote_enc_1 = 0;
  ...
  else if((int)encoder2.getDirection() != 0 || remote_enc_2 == 1)
  {
    remote_enc_2 = 0;
...
{
  if(stepper1.distanceToGo() == 0 && stepper2.distanceToGo() == 0) // only if steppers have stopped update remote status
  {
    Serial.print("Rot_1,");
    Serial.print(RotaryValue_1);
    Serial.print(",");
    Serial.print("Rot_2,");
    Serial.print(RotaryValue_2);
    Serial.print(",");
...
void remoteRun()
{
  ...
  while(Serial.available() > 0)
  {
    ...
    byte receiveID = received_data[2];            // position 2 has the ID

    switch(receiveID)
    {
      case RECEIVE_ID_00: // request for instrument status
        remoteSendStatus();
        break;

      case RECEIVE_ID_01: // rotary encoder 1 up
        encoder1.setPosition(encoder1.getPosition() + 2);
        remote_enc_1 = 1;
        break;

      case RECEIVE_ID_02: // rotary encoder 1 down
        encoder1.setPosition(encoder1.getPosition() - 2);
        remote_enc_1 = 1;
        break;

      case RECEIVE_ID_03: // rotary encoder 2 up
        encoder2.setPosition(encoder2.getPosition() + 2);
        remote_enc_2 = 1;
        break;

      case RECEIVE_ID_04: // rotary encoder 2 down
        encoder2.setPosition(encoder2.getPosition() - 2);
        remote_enc_2 = 1;
        break;

      case RECEIVE_ID_05: // rotary encoder 1 up 100
        encoder1.setPosition(encoder1.getPosition() + 200);
        remote_enc_1 = 1;
        break;

      case RECEIVE_ID_06: // rotary encoder 2 up 100
        encoder2.setPosition(encoder2.getPosition() + 200);
        remote_enc_2 = 1;
        break;
              
      case RECEIVE_ID_07: // custom rotart encoder value sent in 2 bytes [4] and [5]
        encoder_delta = received_data[4] | (received_data[5] << 8);
        //Serial.println(encoder_delta);
        encoder1.setPosition(encoder_delta * 2);
        remote_enc_1 = 1;
        break;
    }
  }
